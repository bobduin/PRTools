...
function varargout = prdataset(a,labels,varargin)
  ...
  if isempty(PRTOOLS_START)
    ...
    if isdeployed
      ...
      disp(['   For more information click <a href="http://37steps.com/prtools">here</a>.'])
      disp(' ')
      try % see whether we can find the most recent version
        ver_old = prversion;
        ver_new = prversion('http://prtools.tudelft.nl/files/prtools_version.txt');
        if ~isempty(ver_new)
          if ver_new > ver_old
      ...
      if isoctave
        disp('   This is a preliminary Octave version.')
        disp('   Please read <a href="http://37steps.com/prtools/octave">release notes</a>.')
        disp(' ')
      end
